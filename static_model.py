...
from datasets.static_dataset import StaticDataset
...
def train_mlp(args):
    ...
    if args.exp == 'static-cic-usb':
        dataset_names = [CIC_2018, USB_2021]
        classes = list(dict.fromkeys(CIC_CLASSES + USB_CLASSES))
        dataset_paths = [CIC_PATH, USB_PATH]
    
    sd = StaticDataset(dataset_names, classes, dataset_paths, args)
    train_idx = 'train'
    test_idx = 'test'

    # Load dataset
    print('\nLoading dataset...')
    datasets = {train_idx: sd.train_dataset, test_idx: sd.test_dataset}
    print(f'Dataset classes: {np.unique(sd.labels_train)}\n')
    ...
    eval_batch_freq = len(dataloaders[train_idx])
...
def main():
    ...
    parser.add_argument('--exp', type=str, required=True, choices=['static-cic-usb'], help='The experimental setup for transfer learning')
